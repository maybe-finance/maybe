class Import < ApplicationRecord
  ...
  def generate_rows_from_csv
    ...
    csv_rows.each do |row|
      rows.create!(
        account: row[account_col_label]&.strip.to_s,
        date: row[date_col_label]&.strip.to_s,
        qty: sanitize_number(row[qty_col_label])&.strip.to_s,
        ticker: row[ticker_col_label]&.strip.to_s,
        exchange_operating_mic: row[exchange_operating_mic_col_label]&.strip.to_s,
        price: sanitize_number(row[price_col_label])&.strip.to_s,
        amount: sanitize_number(row[amount_col_label])&.strip.to_s,
        currency: (row[currency_col_label] || default_currency)&.strip.to_s,
        name: (row[name_col_label] || default_row_name)&.strip.to_s,
        category: row[category_col_label]&.strip.to_s,
        tags: row[tags_col_label]&.strip.to_s,
        entity_type: row[entity_type_col_label]&.strip.to_s,
        notes: row[notes_col_label]&.strip.to_s
      )
    end
  end
