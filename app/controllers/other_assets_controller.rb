class OtherAssetsController < AccountsController
  before_action :set_account, only: [ :update, :show ]

  def new
    @account = Current.family.accounts.other_assets.build(
  ...
  private
    def set_account
      @account = Current.family.accounts.other_assets.find_by(accountable_id: params[:id])
    end
