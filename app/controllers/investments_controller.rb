class InvestmentsController < AccountsController
  before_action :set_account, only: [ :update, :show ]

  def new
    @account = Current.family.accounts.investments.build(
  ...
  private
    def set_account
      @account = Current.family.accounts.investments.find_by(accountable_id: params[:id])
    end
