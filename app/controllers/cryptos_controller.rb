class CryptosController < AccountsController
  layout :with_sidebar

  before_action :set_account, only: [ :update, :show ]

  def new
    @account = Current.family.accounts.cryptos.build(
  ...
  private
    def set_account
      @account = Current.family.accounts.cryptos.find_by(accountable_id: params[:id])
    end
