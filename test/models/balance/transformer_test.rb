...
class Balance::TransformerTest < ActiveSupport::TestCase
  ...
  test "set_absolute_balance splits balance for cash-only accounts (checking, credit cards)" do
    transformer = Balance::Transformer.new(@checking_account)
    valuation = OpenStruct.new(amount: 1000)

    result = transformer.set_absolute_balance(total_balance: valuation.amount, holdings_value: nil)
  ...
  test "set_absolute_balance splits balance for non-cash accounts (property, loans)" do
  ...
  test "set_absolute_balance calculates brokerage cash for mixed accounts (investments, crypto)" do
    transformer = Balance::Transformer.new(@investment_account)
    valuation = OpenStruct.new(amount: 10000)

    result = transformer.set_absolute_balance(total_balance: valuation.amount, holdings_value: 7500)
  ...
  test "set_absolute_balance treats mixed accounts as all-cash when no holdings provided" do
    transformer = Balance::Transformer.new(@investment_account)
    valuation = OpenStruct.new(amount: 10000)

    result = transformer.set_absolute_balance(total_balance: valuation.amount, holdings_value: 0)
  ...
  test "transform_balance applies entries to cash balance for depository accounts" do
    transformer = Balance::Transformer.new(@checking_account)
    entries = [
    ...
    result = transformer.transform_balance(start_cash_balance: 1000, start_non_cash_balance: 0, today_entries: entries)
  ...
  test "transform_balance ignores entries for property accounts" do
    transformer = Balance::Transformer.new(@property_account)
    entries = [
      OpenStruct.new(amount: 1000)  # This could be property tax payment
    ]

    result = transformer.transform_balance(start_cash_balance: 0, start_non_cash_balance: 500000, today_entries: entries)
  ...
  test "transform_balance applies loan payments to principal balance" do
    transformer = Balance::Transformer.new(@loan_account)
    entries = [
      OpenStruct.new(amount: -500)  # Loan payment (negative = reducing debt)
    ]

    result = transformer.transform_balance(start_cash_balance: 0, start_non_cash_balance: 10000, today_entries: entries)
  ...
  test "transform_balance applies entries to cash portion of investment accounts" do
    transformer = Balance::Transformer.new(@investment_account)
    entries = [
      OpenStruct.new(amount: 200)  # Cash withdrawal from brokerage
    ]

    result = transformer.transform_balance(start_cash_balance: 1000, start_non_cash_balance: 5000, today_entries: entries, today_holdings_value: 5000)
  ...
  test "forward_transformation inverts entry flows for asset accounts" do
  ...
  test "reverse_transformation preserves entry flows for asset accounts" do
  ...
  test "transform with no entries returns unchanged balances" do
    ...
    result = transformer.transform_balance(start_cash_balance: 1000, start_non_cash_balance: 0, today_entries: [])
